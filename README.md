...
Problem Statement
----------------------------
It is required to implement a replicated file system. There will be one main server (namingServer) and, data will be partitioned and replicated on multiple replicaServers. This file system allows its concurrent users to perform transactions, while guaranteeing ACID properties.

# Design Assumptions #
- NamingServer server never transfers data.
...
## Client ##
+ Contacts the namingServer for any approving r/w requests.
+ If namingServer approves communicates with replica server(s) to perform the transaction.

# Read / Write Flow #
Read
----------
> 
1. Client contacts the namingServer server requesting read operation with file name.
2. NamingServer replies with a list of all replica servers having a copy of the file.
...
--------
> 
1. Client contacts the namingServer server requesting write operation with file name.
2. If the file is not found in system, NamingServer randomly selects 3 lucky replicas to save the new file and assigns one of them as the primary replica for the new file.
	1. NamingServer initiates a CreateNewFile opearion at each of the lucky replicas.
	2. NamingServer informs the primary replica of the file to take charge of  that file.
3. NamingServer replies to the client with a write acknowledgement containing primary replica location, time stamp, transaction ID.
