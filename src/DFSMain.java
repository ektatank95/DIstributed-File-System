...
public class DFSMain {
    ...
    static void respawnReplicaServers(Master master)throws IOException{
        ...
        StorageLocation replicaLocation;
        String s;

        for (int i = 0; i < n; i++) {
            s = br.readLine().trim();
            replicaLocation = new StorageLocation(i, s.substring(0, s.indexOf(':')) , true);
            StorageServer rs = new StorageServer(i, "./");

            Remote stub = (Remote) UnicastRemoteObject.exportObject(rs, 0);
            registry.rebind("ReplicaClient"+i, stub);

            master.registerReplicaServer(replicaLocation, stub);
    ...
    public  static void customTest() throws IOException, NotBoundException, MessageNotFoundException{
        ...
        StorageServerClientInterface stub = (StorageServerClientInterface) registry.lookup("ReplicaClient"+ackMsg.getLoc().getId());
        ...
        // read here
        List<StorageLocation> locations = c.masterStub.read(fileName);
        ...
        //commit
        StorageLocation primaryLoc = c.masterStub.locatePrimaryReplica(fileName);
        ...
        replicaStub = (StorageServerClientInterface) registry.lookup("ReplicaClient"+replicaLoc.getId());
        fileContent = replicaStub.read(fileName);
        System.err.println("[@CustomTest] data:");
        System.err.println(new String(fileContent.getData()));
