...
public class Master implements Remote, MasterServerClientInterface{
	...
	class HeartBeatTask extends TimerTask {
		...
		public void run() {
			...
			for (StorageLocation replicaLoc : replicaServersLocs) {
				try {
	...
	private Map<String, StorageLocation> primaryReplicaMap;
	//	private Map<Integer, String> activeTransactions; // active transactions <ID, fileName>
	private List<StorageLocation> replicaServersLocs;
	private List<StorageMasterInterface> replicaServersStubs;


	public Master() {
		filesLocationMap = new HashMap<String, List<StorageLocation>>();
		primaryReplicaMap = new HashMap<String, StorageLocation>();
//		activeTransactions = new HashMap<Integer, String>();
		replicaServersLocs = new ArrayList<StorageLocation>();
		replicaServersStubs = new ArrayList<StorageMasterInterface>();
	...
	private void createNewFile(String fileName){
		...
		List<StorageLocation> replicas = new ArrayList<StorageLocation>();
...
//				System.err.println(replicaServersLocs.get(luckyReplicas[i]).isAlive());
	...
	public WriteAck write(String fileName) throws RemoteException, IOException {
		...
		StorageLocation primaryReplicaLoc = primaryReplicaMap.get(fileName);
	 ...
	 * @param replicaStub
	 */
	public void registerReplicaServer(StorageLocation replicaLoc, Remote replicaStub){
		replicaServersLocs.add(replicaLoc);
		replicaServersStubs.add( (StorageMasterInterface) replicaStub);
	}
